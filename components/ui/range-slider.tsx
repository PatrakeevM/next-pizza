...
export const RangeSlider = React.forwardRef(
  (
    ...
    const initialState = Array.isArray(value) ? value : [min, max];
    const [localValues, setLocalValues] = React.useState(initialState);
    ...
    return (
      <SliderPrimitive.Root
        ...
        className={cn(
          "relative flex items-center w-full touch-none select-none mb-6",
          className
        )}
        {...props}
      >
        <SliderPrimitive.Track className="relative w-full h-1 grow overflow-hidden rounded-full bg-primary/20">
